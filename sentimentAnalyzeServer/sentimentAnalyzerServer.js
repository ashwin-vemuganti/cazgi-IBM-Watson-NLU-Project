...
app.get("/url/sentiment", (req,res) => {
    ...
    naturalLanguageUnderstanding.analyze(analyzeParams).then(
        ...
        .catch(err =>{
            return res.send("Could not do the desired sentiment operation" + err);
        });
});
...
app.get("/text/emotion", (req,res) => {
    ...
    naturalLanguageUnderstanding.analyze(analyzeText).then(
            analysisResults=> {
                return res.send(analysisResults.result.keywords[0].emotion,null,2);
            })
            .catch(err => {
                return res.send("Could not do the desired text analyze emotion operation." + err);
            });
});
...
app.get("/text/sentiment", (req,res) => {
    ...
    naturalLanguageUnderstanding.analyze(analyzeText).then(
        ...
        .catch(err => {
            return res.send("Could not do the desired text analyze sentiment operation." + err);
        });
});
